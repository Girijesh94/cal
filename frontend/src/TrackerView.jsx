...
export default function TrackerView({ productToAdd, onProductAdded }) {
  ...
  // Handle incoming product from MyProducts tab
  useEffect(() => {
    if (productToAdd) {
      setIngredientsForm((prev) => {
        // If the first row is empty, replace it
        if (prev.length === 1 && !prev[0].name && !prev[0].quantityGrams) {
          return [{ name: productToAdd.product_name, quantityGrams: productToAdd.serving_size.toString() }];
        }
        // Otherwise append
        return [...prev, { name: productToAdd.product_name, quantityGrams: productToAdd.serving_size.toString() }];
      });
      setEntryMode('ingredients');
      onProductAdded();
    }
  }, [productToAdd, onProductAdded]);
  ...
  // Fetch products when switching to products view
